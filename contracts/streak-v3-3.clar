...
;;
;; Notes:
;; - This is a new contract (immutable once deployed). Deploy as `streak-v3-3`.
...
;; Withdraw STX accumulated by paid mints.
(define-public (withdraw (amount uint) (recipient principal))
  (match (assert-owner)
    ok-val (as-contract (stx-transfer? amount tx-sender recipient))
    err-code (err err-code)
  )
)
